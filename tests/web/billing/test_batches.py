"""
Billing - Batches lifecycle tests.

Covers the full billing batch workflow: create, process, compare,
view history/transactions, run reports, finalize, and download
invoices from closed batches.
"""
import re
...
class TestBatches:
    ...
    Usage:
        pytest tests/web/billing/test_batches.py -v
    """

    # Shared across ordered tests within the same session
    _batch_id = None
    ...
    def test_c59663_comparison(self, driver):
        """C59663 Comparation."""
        page = self._open_batches(driver)
        batch_id = TestBatches._batch_id or ""
    ...
    def test_c59669_finalize_batch(self, driver):
        ...
        page = self._open_batches(driver)
        batch_id = TestBatches._batch_id or ""
        assert batch_id, "Batch ID required - run test_c59659 first"
